...
1. Run `quickstart_SR.sas` using Enterprise Guide (does not seem to work in batch).
   ...
   1. **Gotcha**: If the SAS program crashes, the likely cause is that the `statrep.cfg` file that you edited in the final step of **Install LaTeX Components** is not the copy of the file that TexMaker is looking at for its configuration information (yes, there are multiple copies of this file created in some installs - this makes no sense). To remedy this problem, search `quickstart.log` for the text `statrep.cfg`. If the path you see does not match the one you used, then you will need to repeat the final step of **Install LaTeX Components** using this new path.
